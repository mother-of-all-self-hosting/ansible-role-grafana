...
#
# Tags look like `v<Grafana version>-<release>`:
#
# - if defaults/main.yml points at a Grafana version that has never been
#   released, the release counter restarts at 0 (`v13.0.2-0`)
# - otherwise the counter is incremented (`v13.0.2-1`), but only if something
...
# Keyed on the literal `grafana_version` value, which is what Renovate bumps.
# The image variables derive from it through Jinja, so they cannot be read here.
version="$(sed -nE 's|^grafana_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Grafana version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `13.0.2`), but the tags
# do (`v13.0.2-0`). Stripping any `v` before prepending one keeps this correct
